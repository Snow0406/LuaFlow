...
LuaFlow의 개발 동기는 JSON 기반 컷신 관리의 불편함에서 시작되었습니다. 그냥 유니티의 인스펙터에서 구현현한 컷씬 관리는 너무 귀찮았고 복잡했습니다, 그래서 다른걸 찾다가 Lua 스크립트, JSON을 보았고 루아를 이용해 컷씬 관리를 하면 간지가 날것 같아서 만들게 됬습니다 !!
...
- **싱글톤 패턴**: `GameEntityManager`, `CameraManager`, `LuaCustomActionManager`와 같은 매니저들은 다양한 씬에서 일관된 접근을 위해 싱글톤을 사용합니다.
...
2. 넣고 싶은 Unity 프로젝트에 이 프로젝트의 `Assets/Scripts/Cutscene/` 폴더 넣기
3. 다음 필수 패키지 설치:

#### UniTask 설치

> UniTask는 Unity의 코루틴에 대한 더 효율적인 대안으로, 더 나은 성능과 깔끔한 구문을 제공합니다.

1. 패키지 매니저 창 열기 (Window > Package Manager)
2. 좌측 상단의 "+" 버튼 클릭
3. "Add package from git URL..." 선택
4. `https://github.com/Cysharp/UniTask.git?path=src/UniTask/Assets/Plugins/UniTask` 입력
5. "Add" 클릭

#### NuGetForUnity 설치

> NuGetForUnity를 사용하면 Unity 프로젝트에서 직접 NuGet 패키지를 사용할 수 있습니다.

1. 패키지 매니저 창 열기 (Window > Package Manager)
2. 좌측 상단의 "+" 버튼 클릭
3. "Add package from git URL..." 선택
4. `https://github.com/GlitchEnzo/NuGetForUnity.git?path=/src/NuGetForUnity` 입력
5. "Add" 클릭

#### Lua-CSharp 설치

> Lua-CSharp는 C#과 Lua 간의 원활한 통합을 가능하게 해줍니다 !

**완전한 기능을 위해 NuGetForUnity와 Git URL 두 곳에서 다운받아야 합니다.**

**1. NuGetForUnity를 통한 설치**

1. NuGetForUnity 설치 후, Unity 메뉴에서 NuGet > Manage NuGet Packages 선택
2. 검색창에 "LuaCSharp" 입력
3. 검색 결과에서 "LuaCSharp" 패키지 찾기
4. Install 버튼 클릭
5. 설치가 완료된 후 패키지가 프로젝트에 적용되었는지 확인
...
2. 좌측 상단의 "+" 버튼 클릭
3. "Add package from git URL..." 선택
4. `https://github.com/nuskey8/Lua-CSharp.git?path=src/Lua.Unity/Assets/Lua.Unity` 입력
5. "Add" 클릭
...
│   │   ├── Core/            # 핵심 기능 (CutsceneManager, LuaScriptManager)
│   │   ├── Entity/          # 게임 엔티티 래퍼
│   │   └── Integration/     # 커스텀 액션 및 이벤트 시스템
│   ├── BaseAnimation.cs     # 애니메이션 유틸리티
│   ├── CameraManager.cs     # 카메라 제어 시스템
│   └── GameEntityManager.cs # 엔티티 등록 및 검색
├── Cutscene/
...
> `GameEntityManager`는 서로 다른 씬에서 게임 오브젝트를 등록, 접근 및 관리하는 중앙 집중식 방법을 제공합니다.
